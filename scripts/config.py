...
def validate_stock_code(code: str) -> str:
    ...
    # Already in US format: AAPL.US or BRK.B.US
    if re.match(r"^[A-Z]{1,5}(?:\.[A-Z])?\.US$", code):
        return code
    ...
    # US class shares: BRK.B / BRK-B -> BRK.B.US
    if re.match(r"^[A-Z]{1,5}[.-][A-Z]$", code):
        return f"{code.replace('-', '.')}.US"

    raise ValueError(
        f"Unrecognized stock code format: '{code}'. "
        "Expected: 600887.SH, 000858.SZ, 00700.HK, AAPL.US, BRK.B, or plain digits."
    )
