...
```bash
make verify   # 与 CI 等价：编译检查 + 全量测试 + 覆盖率门禁（提交前跑这个）
make test     # 全量测试
make unit     # 只跑快层，日常迭代用
make cov      # 覆盖率报告与门禁（≥ 74%，基线 76.77%）
...
`main` 与 `develop` 都受保护，**所有改动都必须通过 Pull Request 合入，不能直接 push**。
功能先合进 `develop`（CI 跑全量 + PR 里写手工自测记录），攒到要上线时再带一份独立验收报告合进 `main`；
`main` 每累积 3 条功能合入要补一次全量回归记录。
