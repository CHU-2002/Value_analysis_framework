...
> **当前版本：v3.0-beta** — 证据索引 + 模块化 Agent 结果 + Cross-check + LLM 综合总结。v2 的 Markdown 输出保持兼容，详见 [CHANGELOG_V2.md](CHANGELOG_V2.md)。
...
### 独立商业分析流程（分层 Agent + LLM 综合）

```
/business-analysis {code}
         │
    ┌────▼────────────────┐
    │ evidence/index.json │  证据索引与来源定位
    │ contexts/*.json     │  按模块的上下文预算
    └────┬────────────────┘
         │
    ┌────▼────┬────▼────┬────▼────┐
    ...
    └────┬────┴────┬────┴────┬────┘
         └─────────┼─────────┘
              ┌────▼──────────┐
              │Cross-check    │  冲突、缺口、时点校验
              └────┬──────────┘
                   │
              ┌────▼──────────┐
              │Final Synthesis│  LLM 重新总结与写作
              └────┬──────────┘
                   │
              qualitative_report.md + result.json
```
...
- **Phase 3** — 定性模块 Agent 并行 + Cross-check + Final Synthesis LLM + 定量分析 + 估值，输出 MD + JSON + HTML

Turtle、Value、Valuation 和 Portfolio 下游统一通过解析器选择定性输入：

```bash
.venv/bin/python -m scripts.results.resolve_qualitative \
  --output-dir output/{code}_{company} \
  --ticker {code}
```

解析器优先校验并压缩四个核心模块 `result.json`，整组不可用时才原子回退到 `qualitative_report.md`，输出 `qualitative_input.json`。`data_pack_market.md` 仍是定量计算的必需输入。
...
/value-analysis 600887           # 巴芒段风格价值分析（以 business-analysis 为前置）
```

`/turtle-analysis` 自动执行 Phase 0 → 1A → 1B → 2A → 2B → 3 完整流程。
`/business-analysis` 仅执行定性分析（轻量 WebSearch + Agent Team 并行评估）。
`/value-analysis` 基于 business-analysis 输出，按未来现金流折现 + EE 收购视角生成价值分析报告。
...
│   │   ├── phase2_valuation.md     #   定性调整 + 报告组装
│   │   └── references/
│   └── value/                     # 巴芒段风格价值分析模块
│       ├── coordinator.md          #   入口（/value-analysis）
│       ├── phase2_value_analysis.md #  价值分析执行指令
│       └── references/
│           ├── value_principles.md #  方法论参考
...
│   ├── valuation_engine.py        #   独立估值预计算引擎
│   ├── value_analysis_engine.py   #   value-analysis 预计算引擎
...
本项目是基于开源项目 [terancejiang/Turtle_investment_framework](https://github.com/terancejiang/Turtle_investment_framework)（作者 ying.j，MIT 许可）的衍生作品，在原龟龟投资框架的基础上扩展了价值分析框架（Value Analysis Framework）、组合策略以及结构化定性结果管线。
