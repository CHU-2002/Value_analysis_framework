...
1. **This period's financials** (`Market data:` blocks) — the latest cumulative period plus its prior-year comparable period. `Q1` / `H1` / `Q3` are year-to-date cumulative, so:
   ...
   - 单季 = current cumulative minus the previous cumulative period of the same fiscal year (`H1−Q1`, `Q3−H1`, `FY−Q3`). Never present a single-quarter figure without stating the subtraction.
...
- `report_period` / `comparable_period` are period identifiers such as `2026H1` and `2025H1`; take them from the context metadata rather than inventing them.
...
- Numeric parameters must come from the supplied data. Use `null` when the input cannot support them and record the reason in `quality.warnings`; never estimate.

## Required output

Beyond the standard contract, include in `metrics` at least: the current and comparable period labels, revenue / net profit 同比百分数, the single-quarter figures you derived with their subtraction stated, gross-margin change in percentage points, and operating-cash-flow-to-net-profit. Keep units and periods attached to every metric.
