...
def test_new_snapshots_inputs_preferring_hardlinks(tmp_path):
    ...
    for item in entries.values():
        assert set(item) == {"source_path", "snapshot_path", "sha256", "size_bytes", "hardlinked"}
        assert Path(item["snapshot_path"]).is_file()
        assert item["size_bytes"] == Path(item["snapshot_path"]).stat().st_size

    entry = entries["data_pack_market.md"]
    if not entry["hardlinked"]:
        pytest.skip("filesystem does not support hardlinks")
    assert os.stat(market).st_ino == os.stat(entry["snapshot_path"]).st_ino


def test_new_falls_back_to_copy_when_hardlink_fails(tmp_path, monkeypatch):
    company = _company_dir(tmp_path)
    ...
    monkeypatch.setattr(runs.os, "link", refuse_link)
    result = runs.create_run(
        company,
        ticker="600887.SH",
        company="伊利股份",
    ...
    assert entry["sha256"] == runs.sha256_file(snapshot)

    # A real copy: mutating the source must not leak into the snapshot.
    market.write_text("mutated after snapshot\n", encoding="utf-8")
    assert snapshot.read_text(encoding="utf-8") == "market data\n"
...
def test_adopt_copies_flat_layout_without_deleting_originals(tmp_path):
    ...
    result = runs.adopt_legacy(company, run_id="baseline-1")

    run_path = company / "runs" / "baseline-1"
    assert Path(result["run_dir"]) == run_path.resolve()
    for relative in (
        "run_manifest.json",
...
def test_adopt_rewrites_manifest_paths_into_the_run(tmp_path):
    company = _make_flat_directory(tmp_path)
    runs.adopt_legacy(company, run_id="baseline-2")

    copied = json.loads((company / "runs" / "baseline-2" / "run_manifest.json").read_text(encoding="utf-8"))
    assert copied["inputs"][0]["path"].startswith(str((company / "runs" / "baseline-2").resolve()))
...
def test_adopt_prune_only_runs_after_verification(tmp_path):
    company = _make_flat_directory(tmp_path)
    runs.adopt_legacy(company, run_id="baseline-3", prune=True)
    ...
    for relative in (
        ...
        assert (company / "runs" / "baseline-3" / relative).exists(), relative
...
def test_adopt_aborts_prune_when_verification_fails(tmp_path, monkeypatch):
    ...
    with pytest.raises(runs.LedgerError):
        runs.adopt_legacy(company, run_id="baseline-4", prune=True)
...
def test_adopt_refuses_directory_that_already_has_a_ledger(tmp_path):
    company = _make_flat_directory(tmp_path)
    runs.adopt_legacy(company, run_id="baseline-5")
    with pytest.raises(runs.LedgerError):
        runs.adopt_legacy(company, run_id="baseline-6")
...
def test_export_omits_pdf_and_report_bodies(tmp_path, capsys):
    company = _make_flat_directory(tmp_path)
    runs.adopt_legacy(company, run_id="baseline-export")
    ...
    assert "baseline-export" in markdown
    ...
    assert payload["runs"][0]["run_id"] == "baseline-export"
    ...
    assert json.loads(capsys.readouterr().out)["latest_run"] == "baseline-export"
