...
```bash
make verify      # 与 CI 等价：lint + 全量测试 + 覆盖率 + 追溯门禁 + scope 检查（提交前跑这个）
...
基线（2026-09-20，`main` = `e453255`）：**76.30%**（11044 语句 / 2617 未覆盖），
1432 passed / 3 skipped，约 54s（在 `main` + REQ-006 流程改动上测得）。
