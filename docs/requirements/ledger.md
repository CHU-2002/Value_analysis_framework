...
| 覆盖率洼地补测（`valuation_engine.py` 34%、`portfolio_engine.py` 47%、`split_data_pack.py` 45%、`value_analysis_engine.py` 65%、`report_to_html.py` / `md_to_mobile_html.py` / `generate_available_fields.py` 0%） | `make cov` 基线 2026-09-20，总覆盖率 76.44% | 需先判定这些模块是否属于「一次性脚本」，再决定补测或标注豁免 |
