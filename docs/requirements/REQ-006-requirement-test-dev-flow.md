...
| T5 | CI 提速与简化：8 个 job 收敛为 `ci` + `ci-success`；测试只装 `requirements-test.txt`；`pytest -n auto` 并行；默认只跑 3.12，3.10 改手动核验 | 完成 | 见「维护与合并记录」（PR 号合入后回填） |
...
**T5 交付内容**：`.github/workflows/ci.yml` 重写为两个 job；新增 `requirements-test.txt`；
