...
| [REQ-006](REQ-006-requirement-test-dev-flow.md) | 工程化开发流程（建立与持续维护） | `in-progress`（子需求 `REQ-006.1` 已 `verified`、`REQ-006.2` 未收口） |
...
子需求（`REQ-NNN.S`）见 [`ledger.md`](ledger.md) 的「子需求台账」表；当前有 `REQ-006.1`（`verified`）、
