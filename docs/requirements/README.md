...
2. **AC 编号**：子需求写 `AC-S.n`（`REQ-NNN.2` → `AC-2.1`），父需求自己的判据仍是 `AC-n`。
   两套编号互不顶替：子需求的 `AC-1.1` **不能**当作父需求 `AC-1` 已验收的证据（验收门禁会拒）。
3. **状态**：子需求有自己的状态（同一套状态机），且父需求不得比它最慢的子需求更靠前（§4）。
4. **台账**：`ledger.md` 的「子需求台账」表逐行登记，与文件里的小节严格一致（不重不漏、状态一致）。
5. **验收**：特性分支合 `main` 的批次里写 `REQ-NNN.S`，验收报告就按 `### REQ-NNN.S` 分节逐条打勾；
   写父需求编号则核对父需求自己的 AC。一批里两种可以同时出现。
6. **测试归属**：注释写完整编号 `# 覆盖需求：REQ-NNN.S`。
7. **回归计数**：批量全量回归按**父需求**计一个特性（子需求是同一个特性分支里的批次），阈值不变。
...
| 需求 → 验收 | 批次里的编号决定核对范围：写父需求核对它的 `AC-n`，写子需求只核对那个 `### REQ-NNN.S` 小节的 `AC-S.n` | `scripts/acceptance_gate.py` |
