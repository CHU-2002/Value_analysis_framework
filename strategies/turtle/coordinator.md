...
4. 使用 `scripts/config.py` 校验并保存 canonical `{ticker}`：A股 → `XXXXXX.SH/SZ`；港股 → `XXXXX.HK`；美股 → `AAPL.US`，类别股如 `BRK.B.US`
...
输入解析完成后（`{ticker}` 和 `{company}` 已确定），执行以下检查：
...
{output_dir} = {workspace}/output/{directory_code}_{company}
```

**必须存在**：
1. 可消费的定性输入：优先使用四个核心 `modules/*/result.json`，兼容回退到 `{output_dir}/qualitative_report.md`
   - 执行 `.venv/bin/python -m scripts.results.resolve_qualitative --output-dir "{output_dir}" --ticker "{ticker}" --output "{output_dir}/qualitative_input.json"`
   - 若退出状态为 3 → **停止执行**，输出提示；退出状态 2 表示调用参数错误，必须修正命令：
   ```
   ⚠️ 前置条件不满足：未找到可消费的结构化定性结果或兼容报告
   请先运行 /business-analysis {ticker} 生成定性分析结果。
   ```
   - 不得将不完整 JSON 结果集的参数与旧报告混合

2. `{output_dir}/data_pack_market.md` — Tushare 数据包
   - 若不存在 → **停止执行**，同上提示
   - 若存在 → 复制到 `data_pack_market_current.md` 后通过 `--refresh-market` 刷新副本（§1/§2/§11/§14），保留 manifest 固定的原文件
...
┌─────────────────────────────────────────────────┐
│  前置条件检查                                      │
│  qualitative_input.json source 可用？ → 必须        │
...
│      输入：qualitative_input.json + Agent B 输出    │
│      ↓                                             │
...
只刷新副本，不能改写 manifest 中的原始 `data_pack_market.md`。所有定量步骤读取 `data_pack_market_current.md`；刷新失败时明确披露行情时效性。若全量采集发现影响定性结论的新财报，需要重跑 `/business-analysis`。
...
Task(
  ...
  数据包文件：
    ...
    - {output_dir}/data_pack_market_current.md
...
Task(
  ...
  输入文件：
    - {output_dir}/qualitative_input.json（经校验的定性结果选择）
    - {output_dir}/phase3_quantitative.md（Agent B 输出，含 Step 0 基础信息 + 定量分析）
    - {output_dir}/data_pack_market_current.md（§11 历史价格、§17 预计算值）

  定性参数提取：从 qualitative_input.json 的 parameters 读取；source=legacy 时才读取 qualitative_report.md，
...
| 异常情况 | 处理方式 |
|---------|---------|
| qualitative_input.json source=unavailable | 停止，提示用户运行 /business-analysis |
...
└── output/                                     ← 运行时输出（按标的隔离）
    └── {code}_{company}/
        ...
        ├── data_pack_market.md                 ← /business-analysis 固定证据输入
        ├── data_pack_market_current.md         ← Step A 刷新的行情副本
