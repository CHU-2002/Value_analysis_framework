...
>
> **定性参数来源**：定性参数优先来自 `/business-analysis` 模块的
> `{output_dir}/modules/*/result.json` 和汇总后的结构化参数。
> `qualitative_report.md` 仅作为人工审阅和兼容 fallback。
> （参数定义见 `shared/qualitative/references/output_schema.md`）。
> Agent C 读取 `qualitative_input.json` 中经校验的 JSON 结果卡片，不再解析长篇正文或经由 Agent A 中转。
>
> **值域映射**：通用模块 `moat_rating` 值域为 (强/较强/中/弱)（v1.1），
> 龟龟策略映射为：强→优质，较强→优质，中→中性，弱→负面。
> Agent C 在读取 `qualitative_input.json.parameters`（或 legacy fallback）时执行此映射。
...
## 定性参数（qualitative_input.json）→ Agent C
