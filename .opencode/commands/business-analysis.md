...
**IMPORTANT**: The Read tool may not support PDF files directly. In that case, use `pdfplumber` (available in .venv) to extract text. Extraction priority — these sections **MUST** all be extracted:

| Priority | Section (Chinese) | Typical position | Why |
|----------|-------------------|------------------|-----|
| **P0** | 第三节 管理层讨论与分析 (MD&A) | After 公司简介, before 公司治理 | Core business analysis, strategy, risk discussion |
| **P0** | 第四节 公司治理 (Corporate Governance) | After MD&A | Board structure, executive compensation, internal controls |
| **P0** | **第五节 重要事项** (Significant Matters) | After 公司治理, before 股份变动 | **M&A, major lawsuits, major contracts, related-party transactions, guarantees, stock buybacks — THIS SECTION IS CRITICAL AND MUST NOT BE SKIPPED** |
| P1 | 第二节 公司简介和主要财务指标 | After 重要提示/目录 | Company info, key financial data |
| P1 | 第六节 股份变动及股东情况 | After 重要事项 | Shareholder changes, top shareholders |
| P2 | 第十节 财务报告 (Financial Report) | Usually after 债券相关 | Audit opinion, financial statements, footnotes |

- For A-share reports, the section order is consistent: 重要提示 → 目录 → 公司简介 → **MD&A** → **公司治理** → **重要事项** → 股份变动 → 债券 → **财务报告**
- After extracting, save to text files under output/{code}_{company}/_section_{name}.txt for the Agent to read
- If the Read tool CAN read the PDF, still ensure 第五节 重要事项 is among the read pages
- For the layered pipeline, also run `python3 scripts/pdf_preprocessor.py --pdf output/{code}_{company}/{pdf_filename} --output output/{code}_{company}/pdf_sections.json` so module Agents consume normalized sections rather than the complete PDF. Direct page reads are only for targeted evidence verification.
...
再读取 `shared/qualitative/agents/final_synthesis.md` 启动 Final Synthesis Agent。它由大模型重新撰写执行摘要、六维度综合判断、风险排序和投资启示；不得直接拼接模块正文，也不得读取完整 PDF。输出 `qualitative_report.md` 和 `synthesis/result.json`。
